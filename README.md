...
For production, copy `.env.example` to `.env` and run `./prod.sh`. Docker builds the React app, runs FastAPI privately, and exposes Nginx on **port 5001**. Nginx serves the SPA, serves cutout FITS files from `/home/astrodados4/arandu/data/cutouts` at `/cutouts/`, and proxies `/api` to FastAPI. Cutouts must be arranged as `<dia_source_id>/<science|template|difference>.fits`.
