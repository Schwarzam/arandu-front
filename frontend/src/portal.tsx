...
import { BarElement, CategoryScale, Chart as ChartJS, Legend, LinearScale, PointElement, Tooltip } from 'chart.js'
...
type Enrichment = { enrichment_id: number; dia_source_id: number; enricher_name: string; version: string; value?: { label?: string; score?: number }; enriched_at?: string }
type ObjectDetails = { object: SearchObject; sources: Source[]; enrichments: Enrichment[] }
...
function Calendar() {
  ...
  const dailyTotals = cells.filter((day): day is Date => day !== null).map(day => ({ day, detections: totals[isoDate(day)]?.detections ?? 0 }))
  const histogramData = { labels: dailyTotals.map(({ day }) => String(day.getUTCDate())), datasets: [{ label: 'Detections', data: dailyTotals.map(({ detections }) => detections), backgroundColor: themeColor('--color-accent'), hoverBackgroundColor: themeColor('--color-accent-strong'), borderRadius: 2, maxBarThickness: 28 }] }
  const histogramOptions = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (context: any) => `${context.parsed.y} detections` } } }, scales: { x: { title: { display: true, text: 'UTC day of month' }, grid: { display: false } }, y: { beginAtZero: true, ticks: { precision: 0 }, title: { display: true, text: 'DIA source detections' } } } }
  return <section className="page"><div className="page-heading"><div><span className="eyebrow">Observing activity</span><h2>Calendar</h2><p>DIA source detections recorded each UTC day.</p></div><div className="calendar-controls"><button aria-label="Previous month" className="icon-button" onClick={() => setMonth(value => plusMonths(value, -1))}><ChevronLeft /></button><strong>{monthLabel(month)}</strong><button aria-label="Next month" className="icon-button" onClick={() => setMonth(value => plusMonths(value, 1))}><ChevronRight /></button></div></div>{error && <p className="error">{error}</p>}<section className="calendar-chart"><div><h3>Daily detection distribution</h3><p>Same daily totals shown below.</p></div><div className="histogram" aria-busy={loading}><Bar data={histogramData} options={histogramOptions} /></div></section><div className="calendar" aria-busy={loading}>{weekDays.map(day => <div className="weekday" key={day}>{day}</div>)}{cells.map((day, i) => { if (!day) return <div className="calendar-cell blank" key={`blank-${i}`} />; const key = isoDate(day), total = totals[key], isToday = key === today; return <article className={`calendar-cell ${isToday ? 'today' : ''}`} key={key}><time dateTime={key}>{day.getUTCDate()}</time>{loading ? <span className="loading-dot" /> : <div className="calendar-numbers"><span><b>{total?.detections ?? 0}</b> detections{isToday && ' so far'}</span></div>}</article> })}</div><p className="calendar-note">The outlined day is today. Its count is queried live; earlier days are refreshed daily by the server.</p></section>
}
...
function ObjectDetail({ id, onClose }: { id: number; onClose: () => void }) {
  ...
  const colors: Record<string, string> = { u: themeColor('--color-accent-strong'), g: themeColor('--color-accent'), r: themeColor('--color-text'), i: themeColor('--color-muted'), z: themeColor('--color-accent-strong'), y: themeColor('--color-accent') }
  const filterPalette = [themeColor('--color-accent'), themeColor('--color-accent-strong'), themeColor('--color-text'), themeColor('--color-muted')]
...
function DiscoverySearch() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const result = await api<{ objects: SearchObject[]; description?: string }>('/api/search/discovery', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query, limit: 100 }) })
      setResults(result.objects); setDescription(result.description ?? '')
    } catch (err) { setError(err instanceof Error ? err.message : 'Could not complete the search.') } finally { setLoading(false) }
  }
  return <section className="page"><div className="page-heading"><div><span className="eyebrow">DIA object catalogue</span><h2>Object discovery</h2><p>Search positions, recent detections, and classification results from one place.</p></div></div><form className="discovery-form" onSubmit={submit}><label htmlFor="discovery-query">Search the catalogue</label><div className="discovery-autocomplete"><div className="discovery-input"><Search size={20} /><input id="discovery-query" autoComplete="off" role="combobox" aria-expanded={suggestionsOpen} aria-controls="discovery-suggestions" placeholder="Try 0.1 0.1 10/3600, classification:… or last:25" value={query} onFocus={() => setSuggestionsOpen(true)} onBlur={() => setSuggestionsOpen(false)} onChange={e => { setQuery(e.target.value); setSuggestionsOpen(true) }} /><button disabled={loading}>{loading ? 'Searching…' : 'Search'}</button></div>{suggestionsOpen && matchingSuggestions.length > 0 && <div className="discovery-suggestions" id="discovery-suggestions" role="listbox"><p>Suggested searches</p>{matchingSuggestions.map(item => <button type="button" role="option" key={item.value} onMouseDown={event => event.preventDefault()} onClick={() => { setQuery(item.value); setSuggestionsOpen(false) }}><span><b>{item.title}</b><small>{item.detail}</small></span><code>{item.value}</code></button>)}</div>}</div><div className="search-guides"><button type="button" onClick={() => setQuery('0.1 0.1 10/3600')}><b>Cone</b><span>RA Dec radius, in arcsec</span></button><button type="button" onClick={() => setQuery('classification:candidate_transient')}><b>Class</b><span>Find a classification label</span></button><button type="button" onClick={() => setQuery('last:25')}><b>Recent</b><span>Newest detection records</span></button></div></form>{error && <p className="error">{error}</p>}{results.length > 0 && <div className="result-table-wrap"><table><caption>{description || `${results.length} matching objects`} — select an ID for its light curve, enrichments, and cutouts.</caption><thead><tr><th>Object ID</th><th>RA</th><th>Dec</th><th>Separation</th><th>Sources</th></tr></thead><tbody>{results.map(object => <tr key={object.dia_object_id} className="clickable-row" onClick={() => setObjectId(object.dia_object_id)}><td><button className="object-link">{object.dia_object_id}</button></td><td>{Number(object.ra).toFixed(6)}°</td><td>{Number(object.dec).toFixed(6)}°</td><td>{Number.isFinite(Number(object.separation_arcsec)) ? `${Number(object.separation_arcsec).toFixed(2)}″` : '—'}</td><td>{object.n_dia_sources ?? '—'}</td></tr>)}</tbody></table></div>}{!loading && !error && results.length === 0 && <div className="empty-results"><Crosshair size={25} /><p>Search by sky position, classification, or the newest observations.</p></div>}{objectId !== null && <ObjectDetail id={objectId} onClose={() => setObjectId(null)} />}</section>
}

function Docs() {
  const [documents, setDocuments] = useState<Document[]>([]), [selected, setSelected] = useState(''), [markdown, setMarkdown] = useState(''), [error, setError] = useState(''), [refresh, setRefresh] = useState(0)
  useEffect(() => { api<{ documents: Document[] }>('/api/docs', { cache: 'no-store' }).then(({ documents }) => { setDocuments(documents); setSelected(current => current && documents.some(document => document.path === current) ? current : documents[0]?.path ?? '') }).catch(err => setError(err instanceof Error ? err.message : 'Could not load documentation.')) }, [refresh])
  useEffect(() => { if (!selected) return; setError(''); api<{ markdown: string }>(`/api/docs/${selected}`, { cache: 'no-store' }).then(result => setMarkdown(result.markdown)).catch(err => setError(err instanceof Error ? err.message : 'Could not load this document.')) }, [selected, refresh])
