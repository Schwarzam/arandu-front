...
type DayTotal = { day: string; objects: number; detections: number }
...
function Calendar() {
  ...
  return <section className="page"><div className="page-heading"><div><span className="eyebrow">Observing activity</span><h2>Calendar</h2><p>Distinct DIA objects and detections recorded each UTC day.</p></div><div className="calendar-controls"><button aria-label="Previous month" className="icon-button" onClick={() => setMonth(value => plusMonths(value, -1))}><ChevronLeft /></button><strong>{monthLabel(month)}</strong><button aria-label="Next month" className="icon-button" onClick={() => setMonth(value => plusMonths(value, 1))}><ChevronRight /></button></div></div>{error && <p className="error">{error}</p>}<div className="calendar" aria-busy={loading}>{weekDays.map(day => <div className="weekday" key={day}>{day}</div>)}{cells.map((day, i) => { if (!day) return <div className="calendar-cell blank" key={`blank-${i}`} />; const key = isoDate(day), total = totals[key]; return <article className={`calendar-cell ${key === today ? 'today' : ''}`} key={key}><time dateTime={key}>{day.getUTCDate()}</time>{loading ? <span className="loading-dot" /> : <div className="calendar-numbers"><span><b>{total?.objects ?? 0}</b> objects</span><span><b>{total?.detections ?? 0}</b> detections</span></div>}</article> })}</div><p className="calendar-note">The outlined day is today. Counts are refreshed in the background at most once per UTC day.</p></section>
}
